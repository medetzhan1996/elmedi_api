...
class ReferralForm(forms.ModelForm):
    ...
    class Meta:
        model = Referral
        fields = ['directed_hospital', 'service', 'doctor_full_name', 'icd', 'customer_insurance',
                  'type_appeal', 'place']
    ...
    def __init__(self, iin, hospital=None, *args, **kwargs):
        ...
        print('hospital.....')
        print(self.hospital)
        self.fields['customer_insurance'].queryset = CustomerInsurance.objects.filter(
            customer__iin=iin)

    def save(self, commit=True):
        form = super(ReferralForm, self).save(commit=False)
        form.customer = self.cleaned_data['customer_insurance'].customer
